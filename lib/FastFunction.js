...
class FastFunction extends FunctionDefinition {
            ...
            constructor(fn) {
                ...
                for (const setter of vmArgSetters) {
                    const specPtrDef = setter.type.callback ||
                            ...
                            setter.type.array;
                    if (specPtrDef) {
                        this['argSetter' + i++] = value => setter.func(specPtrDef.makePtr(value));
                    }
                    else if (FastFunction._isArrayType(setter.type)) {
                        this['argSetter' + i++] = value => {
                            setter.func(value.buffer);
                        }
                    }
                    else {
    ...
    _makeAsyncFunction() {
        const vmArgSetters = this.args.map(arg => this._findVMSetterFunc(arg.type));
        const hasPtrArg = Boolean(_(vmArgSetters).filter(setter => FastFunction._isPointerType(setter.type)).head());
        const funcArgs = _.range(vmArgSetters.length).map(n => 'arg' + n);
        ...
        for (let i = 0; i < vmArgSetters.length; i++) {
            ...
            if (FastFunction._isPointerType(setter.type)) {
    ...
    static _isPointerType(type) {
        return type.indirection > 1 || FastFunction._isArrayType(type);
    }

    static _isArrayType(type) {
        return type.indirection === 1 && type.name === 'ArrayType';
    }
}
