...
function Callback(resultType, argTypes, func) {
    ...
    this._name = 'FFICallback' + typeNameCounter++;
...
Callback.prototype._makePtr = function (library) {
    ...
    if (!this._ptr) {
        ...
        if (!ptrFactory) {
            library.callback({ [this._name]: [ this._resultType, this._args ] });
            ptrFactory = library.interface[this._name];
        }
        verify(_.isFunction(ptrFactory));
