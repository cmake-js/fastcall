...
class Library extends native.LibraryBase {
    ...
    _addFunction(func) {
        assert(!this._functions[func.name], `Function ${ func.name } already declared.`);
        this.initialize();
        func.initialize();
    ...
    _addCallback(cb) {
        assert(!this._callbacks[cb.name], `Callback ${ cb.name } already declared.`);
        this.initialize();
        cb.initialize();
        this._callbacks[cb.name] = cb;
        this.interface[cb.name] = cb.getMakeCallbackFunction();
    }
}
