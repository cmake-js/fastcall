...
class Function extends FunctionDefinition {
    ...
    _makeAsyncFunction() {
        const vmArgSetters = this.args.map(arg => this._findVMSetterFunc(arg.type));
        const hasCallbackArg = Boolean(_(vmArgSetters).filter(setter => setter.type.calbacks).head());
        const funcArgs = _.range(vmArgSetters.length).map(n => 'arg' + n);
        let funcBody = hasCallbackArg ? 'const callbacks = [];' : '';
        ...
        if (hasCallbackArg) {
            funcBody += 'return this.callerFunc(this.vm).finally(() => calbacks.length = 0);';
        }
        else {
