...
class Parser {
    ...
    parseFunctionString(def) {
        ...
        let i = 0;
        args = args.map(arg => this._parseDeclaration(arg, 'argument', 'arg' + i++));
        return { resultType, name, args };
    }
    ...
    parseFields(def, keyword) {
        ...
        for (const part of parts) {
            ...
            if (fieldDecl) {
                const decl = this._parseDeclaration(part, 'declaration');            
    ...
    _parseDeclaration(def, name, defaultName) {
        def = def.trim();
        ...
        if (!part2 && !defaultName) {
            assert(false, `${ name } declaration's name name expected.`);
        }
        return {
            name: part2 || defaultName,
            type: this._makeRef(part1)
        };
    }
    ...
    _makeRef(value) {
        if (_.isString(value)) {
            ...
            if (nameMatch.length === 2) {
                ...
                let type = null;
                let propName = null;
                const struct = this.library.structs[name];
                if (struct) {
                    type = struct.type;
                    propName = 'struct';
                }
                else {
                    const union = this.library.unions[name];
                    if (union) {
                        type = union.type;
                        propName = 'union';
                    }
                ...
                if (type) {
                    const starCount = Parser._countStars(value);
                    assert(starCount, 'Passing by struct or unions by value is not supported.');
                    ...
                    if (starCount === 1) {
                        // For auto conversion support:
                        type[propName] = originalType;
                    }
                    return type;
                }
            }
