...
TCallbackFactory fastcall::MakeCallbackFactory(const v8::Local<Object>& callback)
{
    Nan::HandleScope scope;

    auto library = GetValue<Object>(callback, "library");
    auto _loop = GetValue<Object>(library, "_loop");
    auto loop = Unwrap<Loop>(_loop);
    auto dcArgsToCallbackArgs = MakeDCArgsToCallbackArgsFunction(callback);
    auto setDCValue = MakeSetDCValueFunction(callback);
    auto signature = GetSignature(callback);
    char resultTypeCode = signature[signature.size() - 1];
    auto voidPtrType = GetValue<Object>(callback, "_ptrType");
